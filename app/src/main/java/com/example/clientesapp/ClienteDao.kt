...
class ClienteDao(context: Context) {
    ...
    fun insert(cliente: Cliente): String {
       ...
       // db.execSQL(sql)
        ...
        contentValues.put(CLIENTE_EMPRESA, cliente.empresa)
    ...
    private fun clienteFromCursor(cursor: Cursor): Cliente{
        ...
        val empresa = cursor.getString(cursor.getColumnIndex(CLIENTE_EMPRESA))
        return Cliente(id,nome,fone,idade,empresa)
    }
